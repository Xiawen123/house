...
<script language="JavaScript" type="text/javascript" src="../scripts/jquery-1.8.3.js"></script>
  ...
  <script language="JavaScript">
   $(function () {//加载事件
      //添加失去焦点  当前使用的是jquery,后期可以改为vue(了解)
     $("#inputName").blur(function () {
       $.post("checkUsersName",{usersname:$("#inputName").val()},function (data) {//发送异步请求 回调
          if (data.result==1){
               $("#showNotice").html("用户名已被占用！").css("color","red");
          } else {
...
<TABLE class=field>
  ...
  <TR>
    ...
    <TD><INPUT class=text type=text name=name id="inputName"> <span id="showNotice"></span></TD></TR>
  <TR>
