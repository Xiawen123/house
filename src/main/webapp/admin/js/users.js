$(function(){//加载事件
    ...
    $('#dg').datagrid({
        ...
        columns:[[
            ...
            {field:'telephone',title:'用户电话',width:200},
            {field:'isadmin',title:'是否管理员',width:100},
            {field:'age',title:'年龄',width:100},
            {field:'s',title:'操作',width:200,
                formatter: function(value,row,index){
                    //发送异步请求Ajax
                    return "<a href=\"javascript:DelUsers("+row.id+")\" class=\"easyui-linkbutton\" iconCls=\"icon-ok\">删除</a>";
                }
            }
        ]]
    });
...
function SaveUsers(){
    //1.获取数据，发送异步请求实现添加
    /* $post("addUsers",{"name":$("name1").val()},function (data) {
         alert(data)
     },"json")*/
...
function updateDialog(){
    ...
    if(SelectRows.length==1){
        ...
        //发异步请求查询数据库
        $.post("getOneUsers",{"id":SelectRows[0].id},function(data){
...
//删除单行的
function  DelUsers(obj) {  //传的是编号
    //发送异步请求实现删除
    $.messager.confirm('提示框', '确定删除吗？', function(r){
        if (r){
            $.post("deleteUsers",{"id":obj},function(data){// data是json
                if(data.result==1){
                    //实现datagrid的刷新
                    $('#dg').datagrid('reload');
                }else{
